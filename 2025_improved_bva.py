...
def write_to_excel(result_dict, folder):
    ...
    file_path = f'{output_dir}/sequential_amo/results_{date}_2025.xlsx'
...
def solve_from_instance(N, tasks, M=None):
    ...
    solver.process(rounds=1, block=True, cover=True, decompose=True,
                    elim=True, probe=True, probehbr=False, subsume=True,
                    vivify=False, freeze=[])
    sat = solver.solve()
