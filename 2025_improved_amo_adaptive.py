...
def write_to_excel(result_dict, folder):
    ...
    file_path = f'{output_dir}/adaptive_amo/results_{date}_2025.xlsx'
...
def amo_pairwise(cnf: CNF, lits):
    """
    Pairwise (naive) encoding.
    - Clauses: O(n²)
    - Aux vars: 0
    - Best for: n <= 6 (very small constraints)
    """
    n = len(lits)
    for i in range(n):
...
def amo_product(cnf: CNF, pool: VarPool, lits):
    """
    Product encoding (AMO_PE from paper).
    - Clauses: O(n)
    - Aux vars: O(√n)
    - Best for: n > 6 (medium to large constraints)
    """
    global _amo_counter
    _amo_counter += 1
    uid = _amo_counter
...
def amo_adaptive(cnf: CNF, pool: VarPool, lits):
    """
    ADAPTIVE AMO: Choose best encoding based on constraint size.
    
    Threshold analysis:
    - n <= 6:  Pairwise has fewer total clauses (no aux overhead)
    - n > 6:   Product encoding becomes more efficient
    """
    n = len(lits)
    if n <= 1:
        return
    ...
    # Adaptive selection
    if n <= 6:
        # Small constraint: use pairwise (no aux vars)
        amo_pairwise(cnf, lits)
    else:
        # Medium/large constraint: use product
        amo_product(cnf, pool, lits)
...
class SchedulingEncoder:
    ...
    def per_task_exactly_one(self):
        for i in range(self.N):
            ...
            amo_adaptive(self.cnf, self.pool, lits)
...
def process_input_dir(input_dir, folder):
    ...
    for filename in os.listdir(input_dir):
        ...
        result_dict = {"ID": id, "Problem": filename, "Type": "biclique_improved_amo_adaptive"}
